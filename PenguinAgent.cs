...
public class PenguinAgent : Agent
{
    ...
    {
        // Log the actionBuffers for debugging
        Debug.Log($"ActionBuffers: {actionBuffers}");

        // Convert the first action to forward movement
        float forwardAmount = actionBuffers.DiscreteActions[0];
        Debug.Log($"Received forward action: {forwardAmount}");

        // Convert the second action to turning left or right
        float turnAmount = 0f;
        int turnAction = actionBuffers.DiscreteActions[1];
        Debug.Log($"Received turn action: {turnAction}");
        ...
        // Log the determined movements
        Debug.Log($"Forward Amount: {forwardAmount}, Turn Amount: {turnAmount}");
